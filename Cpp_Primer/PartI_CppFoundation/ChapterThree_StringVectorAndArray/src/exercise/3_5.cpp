...
void Exercise_3_5::exercise_3_32() {
    ...
    int array_1[i];
    for (int j = 0; j < i; ++j) { array_1[j] = array[j]; }
    ...
    i = 0;
    for (auto &c : vector_1) { c = i++; }
    vector_2.reserve(vector_1.size());
    for (auto j : vector_1) { vector_2.push_back(j); }
    for (int j = 0; j < vector_2.size(); ++j) { std::cout << vector_2[j] << " "; }
}
...
void Exercise_3_5::exercise_3_35() {
    ...
    for (auto i = array; i != std::end(array); ++i) { *i = 0; }
