...
初始化和对const的引用
初始化常量引用时运行用任意表达式作为初始值，只要其表达式能够转换为引用类型。运行常量引用绑定非常量的对象、字面值或一般表达式。（
**这也是“引用的类型必须与所引用的类型一致”的两个例外之一**）
...
回到标题，何为“顶层const”？从上述我们已经了解const定义位置的不同会产生不同结果，所以我们在此下定义：**顶层const（top-level
const)表示指针本身是个常量，底层const(low-level const)表示所指对象是一个常量**。
