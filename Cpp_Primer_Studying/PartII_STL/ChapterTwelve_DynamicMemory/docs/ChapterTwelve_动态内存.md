...
包括所有标准库类在内的很多C++类都定义了析构函数，通过析构函数清理对象使用的资源。但是并不是所有的类都是这样良好的定义的，对于某些类需要用户显式地释放所使用的任何资源。对于这种类称为<b>哑类</b>。
...
对于上述出现的情况，通过使用shared_ptr修改默认的<b>删除器</b>实现对connection的智能管理：
...
>
> - 不使用相同的内置指针初始化（或reset）<b>多个</b>智能指针
> - 不delete get()返回的指针
...
<b>动态分配一个空数组是合法的</b>，但是指向该空数组的指针不能解引用。
