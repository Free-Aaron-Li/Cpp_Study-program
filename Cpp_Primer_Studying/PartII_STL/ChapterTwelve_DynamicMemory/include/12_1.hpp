...
public:
   ...
   * 练习12.5
   * @description
   * 我们未编写接受一个initializer_list
   * explicit参数的构造函数。讨论这个设计策略的优点和缺点。
   */
  /*
   ...
   * d.
   * 不合法，将普通指针交由智能指针管理，其引用计数为1,当函数结束，引用计数为0并释放p所指向的内存，p成为空悬指针。
   * */
   ...
   * a. 不合法，无法将int转换为指针
   * b.
   * 合法，从程序结构上合法，但是逻辑上非法。pi指向的非动态内存，智能指针无法释放非动态内存
   * c. 合法
   * d. 与b项一致
   * e. 合法
   ...
   * 练习12.22
   * @description
   * 为了能让StrBLobPtr使用const
   * StrBlob，你觉得应该如何修改？定义一个名为ConstStrBlobPtr的类，使其能够指向const
   * StrBlob。
   */
