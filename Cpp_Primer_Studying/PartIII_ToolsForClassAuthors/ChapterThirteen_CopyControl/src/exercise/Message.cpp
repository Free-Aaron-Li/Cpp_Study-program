...
Message::Message(const Message& obj) : _contents(obj._contents), _folders(obj._folders) {
    add_to_Folders(obj);
}
...
void
Message::add_to_Folders(const Message& message) {
...
void
Message::swap(Message& lhs, Message& rhs) {
    using std::swap;
...
void
Message::add_Folder(Folder& folder) {
    _folders.insert(&folder);
}

void
Message::remove_Folder(Folder& folder) {
    _folders.erase(&folder);
}
