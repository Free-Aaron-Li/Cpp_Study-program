...
Has_Ptr &Has_Ptr::operator=(const Has_Ptr &obj) {
...
bool Like_value_HasPtr::operator<(const Like_value_HasPtr &obj) const {
  return *_ps < *obj._ps;
}

std::string Like_value_HasPtr::operator*() const { return *_ps; }
...
Like_pointer_HasPtr::operator=(const Like_pointer_HasPtr &obj) {
  ...
  if (--*_use == 0) { // 2. decrement the reference count of this object(
                      // operator on the left)
    delete _ps; // 2.1 if there are on have other users, release the assigned
