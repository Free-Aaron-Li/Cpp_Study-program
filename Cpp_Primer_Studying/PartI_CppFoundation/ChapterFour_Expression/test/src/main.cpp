...
void test_RvalueAndLvalue() {
   ...
   */
  int *p_1 = &number_1;
  std::cout << "the pointer is " << p_1
            << " and the value of the pointer to the object is " << *p_1
            << "\n ";

  /**
   * 内置解引用运算符、下标运算符、迭代器解引用运算符
   * 、string和vector的下标运算符的求值结果都是左值。
   */
  std::cout << "the p_1 is " << p_1
            << " and the value of the pointer to the number_1 object is "
            << *p_1 << std::endl;
  ...
  std::cout << "the value of the iterator to the vector_1 object is "
            << *iterator_1 << "\n";
  ...
  decltype(number_3 = number_1 + number_2) i = number_1; /* 使用decltype关键字，如果其作用于一个表达式，返回一个引用类型，例如这里的int
                                                            & */
  int *p_2 = &number_1;
  int *p_3 = &number_2;
